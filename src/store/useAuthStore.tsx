...
interface UseAuthStore {
  ...
  signUp: (email: string, password: string, username: string, navigate: (path: string) => void) => Promise<void>;
  signIn: (email: string, password: string, navigate: (path: string) => void) => Promise<void>;
...
export const useAuthStore = create<UseAuthStore>((set) => ({
  ...
  signUp: async (email, password, username, navigate) => {
    ...
    if (loginRegex.test(username) && emailRegex.test(email) && passwordRegex.test(password)) {
      try {
  ...
  forgotPassword: async (email) => {
    ...
    if (emailRegex.test(email)) {
      try {
        const response = await axios.post("https://cookhub.space/api/v1/auth/send_password_link", {
          email,
        });
